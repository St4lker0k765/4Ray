...
ENGINE_API uSpatialDB*		g_SpatialSpace			= NULL;
ENGINE_API uSpatialDB*		g_SpatialSpacePhysic	= NULL;
...
//////////////////////////////////////////////////////////////////////////
ISpatial::ISpatial			(uSpatialDB* space)
{
...
uSpatialDB::uSpatialDB()
#ifdef PROFILE_CRITICAL_SECTIONS
	:cs(MUTEX_PROFILE_ID(uSpatialDB))
...
void			uSpatialDB::initialize(Fbox& BB)
{
	if (0==m_root)			
	{
		// initialize
...
ISpatial_NODE*	uSpatialDB::_node_create		()
{
	stat_nodes	++;
...
void			uSpatialDB::_node_destroy(ISpatial_NODE* &P)
{
	VERIFY						(P->_empty());
	stat_nodes					--;
...
void			uSpatialDB::_insert	(ISpatial_NODE* N, Fvector& n_C, float n_R)
{
...
void			uSpatialDB::insert		(ISpatial* S)
{
	cs.Enter			();
#ifdef DEBUG
	stat_insert.Begin	();
...
void			uSpatialDB::_remove	(ISpatial_NODE* N, ISpatial_NODE* N_sub)
{
	if (0==N)							return;
...
void			uSpatialDB::remove		(ISpatial* S)
{
	cs.Enter			();
#ifdef DEBUG
	stat_remove.Begin	();
...
void			uSpatialDB::update		(u32 nodes/* =8 */)
{
#ifdef DEBUG
	if (0==m_root)	return;
	cs.Enter		();
