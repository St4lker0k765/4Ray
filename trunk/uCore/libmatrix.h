...
public:
	...
	// Class members
	__forceinline	SelfRef	set			(const Self &a) 
	{
		i.set(a.i);
		j.set(a.j);
		k.set(a.k);
	...
	__forceinline	SelfRef	identity	(void) 
	{
	...
	inline	SelfRef	rotation	(const _quaternion<T> &Q);
	__forceinline	SelfRef	mk_xform	(const _quaternion<T> &Q, const Tvector &V);

	// Multiply RES = A[4x4]*B[4x4] (WITH projection)
	__forceinline	SelfRef	mul			(const Self &A,const Self &B)
	{
	...
	__forceinline	SelfRef	mul_43		(const Self &A,const Self &B)
	{
		VERIFY	((this!=&A)&&(this!=&B));
	...
	inline	SelfRef	mulA_44		( const Self &A )			// mul after 
	{
    	Self B; B.set( *this ); 	mul		( A, B );
		return *this;
    };
	inline	SelfRef	mulB_44		( const Self &B )			// mul before
	{
		Self A; A.set( *this ); 	mul		( A, B );
		return *this;
	};
	__forceinline	SelfRef	mulA_43		( const Self &A )			// mul after (no projection)
	{
    	Self B; B.set( *this ); 	mul_43	( A, B );
		return *this;
    };
	__forceinline	SelfRef	mulB_43		( const Self &B )			// mul before (no projection)
	{
		Self A; A.set( *this ); 	mul_43	( A, B );
		return *this;
	};
	inline	SelfRef	invert		( const Self &a )	{		// important: this is 4x3 invert, not the 4x4 one
	...
	inline	bool	invert_b	( const Self &a )	{		// important: this is 4x3 invert, not the 4x4 one
		// faster than self-invert
		T fDetInv = ( a._11 * ( a._22 * a._33 - a._23 * a._32 ) -
			a._12 * ( a._21 * a._33 - a._23 * a._31 ) +
			a._13 * ( a._21 * a._32 - a._22 * a._31 ) );
	...
	inline	SelfRef	transpose	(const Self &matSource)	// faster version of transpose
	{
	...
	inline	SelfRef	translate	(T _x, T _y, T _z ) // setup translation matrix
	{	
	...
	inline	SelfRef	translate_over(T _x, T _y, T _z) // modify only translation
	{	
		c.set	(_x,_y,_z);							
		return *this;
	}
	...
	inline	SelfRef	scale		(T x, T y, T z )	// setup scale matrix
	{	
	...
	inline	SelfRef	rotateX		(T Angle )				// rotation about X axis
	{
		T cosa	= cos(Angle);
		T sina	= sin(Angle);
		i.set		(1,		0,		0,		0);
	...
	inline	SelfRef	rotateY		(T Angle )				// rotation about Y axis
	{
		T cosa	= cos(Angle);
		T sina	= sin(Angle);
		i.set		(cosa,	0,	   -sina,	0);
	...
	inline	SelfRef	rotateZ		(T Angle )				// rotation about Z axis
	{
		T cosa	= cos(Angle);
		T sina	= sin(Angle);
		i.set		(cosa,	sina,	0,		0);
	...
	inline	SelfRef	rotation	( const Tvector &vdir, const Tvector &vnorm )	{
		...
		m[0][0] = vright.x;	m[0][1] = vright.y;	m[0][2] = vright.z; m[0][3]=vright.w;
		m[1][0] = vnorm.x;	m[1][1] = vnorm.y;	m[1][2] = vnorm.z;	m[1][3]=vnorm.w;
		m[2][0] = vdir.x;	m[2][1] = vdir.y;	m[2][2] = vdir.z;	m[2][3]=vdir.w;
		m[3][0] = 0;		m[3][1] = 0;		m[3][2] = 0;		m[3][3]=1;
		return *this;
	}

	inline	SelfRef	mapXYZ		()	{i.set(1, 0, 0, 0);j.set(0, 1, 0, 0);k.set(0, 0, 1, 0);c.set(0, 0, 0, 1);	return *this; }
	inline	SelfRef	mapXZY		()	{i.set(1, 0, 0, 0);j.set(0, 0, 1, 0);k.set(0, 1, 0, 0);c.set(0, 0, 0, 1);	return *this; }
	inline	SelfRef	mapYXZ		()	{i.set(0, 1, 0, 0);j.set(1, 0, 0, 0);k.set(0, 0, 1, 0);c.set(0, 0, 0, 1);	return *this; }
	inline	SelfRef	mapYZX		()	{i.set(0, 1, 0, 0);j.set(0, 0, 1, 0);k.set(1, 0, 0, 0);c.set(0, 0, 0, 1);	return *this; }
	inline	SelfRef	mapZXY		()	{i.set(0, 0, 1, 0);j.set(1, 0, 0, 0);k.set(0, 1, 0, 0);c.set(0, 0, 0, 1);	return *this; }
	inline	SelfRef	mapZYX		()	{i.set(0, 0, 1, 0);j.set(0, 1, 0, 0);k.set(1, 0, 0, 0);c.set(0, 0, 0, 1);	return *this; }

	inline	SelfRef	rotation	( const Tvector &axis, T Angle )	
	{
		T Cosine	= _cos(Angle);
		T Sine		= _sin(Angle);
	...
	inline	SelfRef	mul( const Self &A, T v )	{
	...
	inline	SelfRef	div( const Self &A, T v )	{
		VERIFY(_abs(v)>0.000001f);
		return mul(A,1.0f/v);
	}
	inline	SelfRef	div( T v )					{
		VERIFY(_abs(v)>0.000001f);
		return mul(1.0f/v);
	}
	// fov
	inline	SelfRef	build_projection		(T fFOV, T fAspect, T fNearPlane, T fFarPlane)	{
		return build_projection_HAT		(tanf(fFOV/2.f),fAspect,fNearPlane,fFarPlane);
	}
	// half_fov-angle-tangent
	inline	SelfRef	build_projection_HAT	(T HAT, T fAspect, T fNearPlane, T fFarPlane) 
	{
	...
	inline	SelfRef	build_projection_ortho	(T w, T h, T zn, T zf)
	{
	...
	inline	SelfRef	inertion(const Self &mat, T v)
	{
		T iv = 1.f-v;
		for (int i=0; i<4; i++)
		{
	...
	__forceinline	SelfRef	setHPB	(T h, T p, T b)
	{
	...
	inline	SelfRef	setXYZ	(T x, T y, T z)	{return setHPB(y,x,z);}
	inline	SelfRef	setXYZ	(Tvector& xyz)	{return setHPB(xyz.y,xyz.x,xyz.z);}
	inline	SelfRef	setXYZi	(T x, T y, T z)	{return setHPB(-y,-x,-z);}
	inline	SelfRef	setXYZi	(Tvector& xyz)	{return setHPB(-xyz.y,-xyz.x,-xyz.z);}
	//
	inline	void	getHPB	(T& h, T& p, T& b) const
	{
	...
	inline	void	getHPB	(Tvector& hpb) const{getHPB(hpb.x,hpb.y,hpb.z);}
	inline	void	getXYZ	(T& x, T& y, T& z) const{getHPB(y,x,z);}
	inline	void	getXYZ	(Tvector& xyz) const{getXYZ(xyz.x,xyz.y,xyz.z);}
	inline	void	getXYZi	(T& x, T& y, T& z) const{getHPB(y,x,z);x*=-1.f;y*=-1.f;z*=-1.f;}
	inline	void	getXYZi	(Tvector& xyz) const{getXYZ(xyz.x,xyz.y,xyz.z);xyz.mul(-1.f);}
};
...
extern UCORE_API Fmatrix	Fidentity;
extern UCORE_API Dmatrix	Didentity;
