...
public:
	...
	{
		CopyMemory(this,&a,12*sizeof(float));
		return *this;
	}
	...
	{
		matrix_43T a;
		CopyMemory(&a,this,12*sizeof(float));					// save matrix
		transpose(a);
		return *this;
	}
    ...
    inline SelfRef MxM(SelfCRef M1, SelfCRef M2)
    {
      ...
      m[0][3] = (   M1.m[0][0] * M2.m[0][3] +
                    M1.m[0][1] * M2.m[1][3] +
                    M1.m[0][2] * M2.m[2][3]);
      m[1][3] = (   M1.m[1][0] * M2.m[0][3] +
                    M1.m[1][1] * M2.m[1][3] +
                    M1.m[1][2] * M2.m[2][3]);
      m[2][3] = (   M1.m[2][0] * M2.m[0][3] +
                    M1.m[2][1] * M2.m[1][3] +
                    M1.m[2][2] * M2.m[2][3]);
	  return *this;
	}
...
//--------------------------------------------------------------------------------
	...
	inline	void transform_dir		(_vec2<T> &dest, const _vec2<T> &v)	const 	// preferred to use
	{
		dest.x = v.x*_11 + v.y*_21;
		dest.y = v.x*_12 + v.y*_22;
		dest.z = v.x*_13 + v.y*_23;
		dest.w = v.x*_14 + v.y*_24;
	}
	inline	void transform_dir		(_vec2<T> &v) const
	{
		_vec2<T>		res;
		transform_dir	(res,v);
		v.set			(res);
...
typedef		matrix_43T<float>	Fmatrix43;
typedef		matrix_43T<double>	Dmatrix43;

template <class T>
BOOL	_valid			(const matrix_43T<T>& m)		
{ 
	return 
		_valid(m.i)&& 
		_valid(m.j)&&
