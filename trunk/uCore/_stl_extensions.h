...
public:
	...
	struct rebind			{	typedef ualloc<_Other> other;	};
public:
							pointer					address			(reference _Val) const					{	return (&_Val);	}
							const_pointer			address			(const_reference _Val) const			{	return (&_Val);	}
													ualloc			()										{	}
													ualloc			(const ualloc<T>&)						{	}
	template<class _Other>							ualloc			(const ualloc<_Other>&)					{	}
	template<class _Other>	ualloc<T>&				operator=		(const ualloc<_Other>&)					{	return (*this);	}
...
template<class _Ty,	class _Other>	inline	bool operator==(const ualloc<_Ty>&, const ualloc<_Other>&)		{	return (true);							}
template<class _Ty, class _Other>	inline	bool operator!=(const ualloc<_Ty>&, const ualloc<_Other>&)		{	return (false);							}

namespace std
{
	template<class _Tp1, class _Tp2>	inline	ualloc<_Tp2>&	__stl_alloc_rebind(ualloc<_Tp1>& __a, const _Tp2*)	{	return (ualloc<_Tp2>&)(__a);	}
	template<class _Tp1, class _Tp2>	inline	ualloc<_Tp2>	__stl_alloc_create(ualloc<_Tp1>&, const _Tp2*)		{	return ualloc<_Tp2>();			}
};

// vector
template	<typename T, typename allocator = ualloc<T> >
...
// vector<bool>
template <>
class u_vector<bool,ualloc<bool> >	: public std::vector<bool,ualloc<bool> > {
private:
	typedef std::vector<bool,ualloc<bool> > inherited;
...
template	<typename T, typename allocator = ualloc<T> >									class	u_list 			: public std::list<T,allocator>			{ public: u32 size() const {return (u32)__super::size(); } };
template	<typename K, class P=std::less<K>, typename allocator = ualloc<K> >				class	u_set			: public std::set<K,P,allocator>		{ public: u32 size() const {return (u32)__super::size(); } };
template	<typename K, class P=std::less<K>, typename allocator = ualloc<K> >				class	u_multiset		: public std::multiset<K,P,allocator>	{ public: u32 size() const {return (u32)__super::size(); } };
template	<typename K, class V, class P=std::less<K>, typename allocator = ualloc<std::pair<K,V> > >	class	u_map 			: public std::map<K,V,P,allocator>		{ public: u32 size() const {return (u32)__super::size(); } };
template	<typename K, class V, class P=std::less<K>, typename allocator = ualloc<std::pair<K,V> > >	class	u_multimap		: public std::multimap<K,V,P,allocator>	{ public: u32 size() const {return (u32)__super::size(); } };
