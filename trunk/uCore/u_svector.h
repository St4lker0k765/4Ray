...
public:
	...
	inline reference		operator[] (u32 id)			{ VERIFY(id<count); return array[id];	}
	inline const_reference	operator[] (u32 id)	const	{ VERIFY(id<count); return array[id];	}
	...
	inline void		assign(iterator p, int c) { VERIFY(c>0 && c<dim); memcpy(array,p,c*sizeof(value_type)); count=c; }
